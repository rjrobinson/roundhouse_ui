...
module RoundhouseUi
  ...
  class FilterQuery
    ...
    #
    # `%` only. NOT `_`: SQL's single-character wildcard would silently turn
    # `queue=default_low` into a pattern matching `defaultXlow`, and every Ruby name
    # in this console is underscore-dense. A wildcard nobody typed is the one thing
    # that must never happen here, because the same predicate scopes a Delete.
    #
    # An index walk rather than a Regexp. The pattern comes from a URL and is matched
    # against every entry in a set that can hold fifty thousand of them, so a
    # backtracking engine is both a performance and a ReDoS question. This is linear
    # in the value and provably terminates.
    class Pattern
      MAX_WILDCARDS = 6
      ...
      # Literal characters the pattern insists on. Zero means it matches every
      # possible value, which is not a filter — see the refusal in the parser.
    ...
    attr_reader :klass, :error, :queue, :tag, :text, :raw, :message, :ignored
    ...
    class << self
      ...
      # `keys` restricts which facets this PAGE honours. Errors groups by class and
      # error, so queue= has nothing to apply to there — and a facet that renders as
      # a pill while filtering nothing is precisely the invisible/phantom filter this
      # whole bar exists to prevent. Out of scope refuses, and says where it works.
      ...
      # Old links and bookmarks. `?class=X&error=Y` predates the grammar, and
      # find_like_link, the tag chips and the queue pills all emitted it — so
      # somebody's saved URL, and somebody's shared one, is that shape.
      #
      # Folded by RE-PARSING rather than by assignment. build() does not validate,
      # so a legacy `?error=A"B` assigned straight in would produce a query whose
      # own to_s the parser then refuses — displayed one filter, applied another.
      # Going back through parse means nothing can enter that the grammar would
      # not accept, and conflicts (`?q=class=A&class=B`) hit the parser's existing
      # refusal instead of a second, differently-written precedence rule.
      LEGACY = { class: "class", error: "error", queue: "queue", tag: "tag" }.freeze
    ...
    # A matcher per facet, compiled once. `nil` where the value has no `%`, which is
    # the overwhelming case and stays a plain string comparison.
    ...
    # Does this facet's value match? Exact when there is no wildcard — `queue=default`
    # must never also select `default_low`, which is the invariant the whole facet
    # design rests on.
    ...
    # Was any facet dropped? A degraded query browses normally — the point of
    # dropping rather than refusing — but authorises no bulk action, because the
    # filter that survived selects a SUPERSET of what was asked for.
    ...
    # What the search box shows. Canonical when the query parsed, but the RAW input
    # when it did not: a refused query has no canonical form, and rendering the
    # empty one would swallow the typo along with any chance of fixing it.
    ...
    # The facet region alone, in KEYS order. Public because from_params splices
    # more facets in here — between the facets and the text, never after it.
    def facet_parts
      parts = []
    ...
    class Parser
      ...
      def read_key
        ...
        # Known everywhere, not honoured here. A distinct refusal from "unknown",
        # because the fix is different: go to the page where it applies.
      ...
      def value_ok?(value)
        ...
        # `*` and `?` still refuse, and the message points at `%`. Two spellings for
        # one idea is how you end up with one of them silently doing nothing.
        return false if v.match?(/[*?]/)
        # A pattern with no literal characters matches every possible value. It is
        # not a filter, but it renders as a pill that looks like one — the unfiltered
        # bulk hole wearing a facet's clothing. Refused, so it selects nothing.
        pattern = Pattern.for(v)
        return false if pattern && (pattern.literals.zero? || pattern.too_many?)
      ...
      def bad_value_message(key, value)
        ...
        if v.match?(/[*?]/)
          return "Use % for a wildcard, not #{v[/[*?]/]} — for example " \
                 "#{key}=#{v.tr('*?', '%%')}."
        end
        if (pattern = Pattern.for(v))
          if pattern.literals.zero?
            return "#{key}=#{v} matches everything, which is not a filter. Give it " \
                   "something to match, like #{key}=Something%, or drop it."
          end
          if pattern.too_many?
            return "#{key}= takes at most #{Pattern::MAX_WILDCARDS} wildcards."
          end
        end
      ...
      def refuse(message)
        if @out_of_scope
          message = %(#{@out_of_scope}= does not narrow anything on this page. ) +
                    %(Filters here: #{@keys.reject { |k| k == "text" }.join(', ')}.)
        elsif @unknown
